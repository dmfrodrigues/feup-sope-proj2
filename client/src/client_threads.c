...
int client_send_request(char *public_fifo_name, message_t request) {
    ...
    if (write(public_fifo_filedes, &request, sizeof(message_t)) == -1) 
                                    return EXIT_FAILURE;        // Write message to fifo
...
void *client_execute_thread(void *arg) {
    ...
    *ret = EXIT_SUCCESS;
    ...
    if(atomic_lli_inc(num_threads))          // Increment num_threads (as we are entering a new thread)
    ...
    if(sprintf(private_fifo_path, "/tmp/%d.%lu", getpid(), pthread_self()) < 0)
        {*ret = EXIT_FAILURE; return ret; }
    ...
    if ((req = client_send_request(args->public_fifoname, m))){ 
        if (req == 2){
            ...
            if(client_thread_args_dtor(args)){*ret = EXIT_FAILURE; return ret; } // Free arguments
            free(args);
